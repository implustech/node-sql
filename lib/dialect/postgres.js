...
Postgres.prototype.visit = function(node, config) {
  switch(node.type) {
    case 'QUERY'           : return this.visitQuery(node, config);
...
Postgres.prototype.visitQuery = function(queryNode, config) {
  if (this._queryNode) return this.visitSubquery(queryNode,dontParenthesizeSubQuery(this._queryNode, config));
 ...
 * @returns {boolean}
 */
function dontParenthesizeSubQuery(parentQuery, config){
	if (!parentQuery) return false;
	if (parentQuery.nodes.length === 0) return false;
	if (['INSERT', 'REPLACE'].indexOf(parentQuery.nodes[0].type) === -1 && !(config && config.joinFromQuery))  return false;
	return true;
}
